<div class="box disk-info">
	...
	<?php
	$v_free_space   = round( disk_free_space( '/' ) / 1024 / 1024 / 1024, 1 );
	$v_total_space  = round( disk_total_space( '/' ) / 1024 / 1024 / 1024, 0 );
	$v_used_space   = round( $v_total_space - $v_free_space, 1 );
	$v_used_percent = round( ( $v_used_space / $v_total_space ) * 100, 0 );
	$v_low_space    = ( 200 > ( $v_free_space * 1024 ) );
	?>
	<div class="disk">
		...
		<div class="disk-footer-meta">
			...
			<div class="disk-free"><small><?php echo $v_free_space; ?> GB</small></div>
		</div>
	</div>

	<?php
	$p_free_space   = round( disk_free_space( '.' ) / 1024 / 1024 / 1024, 1 );
	$p_total_space  = round( disk_total_space( '.' ) / 1024 / 1024 / 1024, 0 );
	$p_used_space   = round( $p_total_space - $p_free_space, 1 );
	$p_used_percent = round( ( $p_used_space / $p_total_space ) * 100, 0 );
	$p_low_space    = ( 200 > ( $p_free_space * 1024 ) );
	?>
	<div class="disk">
		<div class="disk-header-meta">
